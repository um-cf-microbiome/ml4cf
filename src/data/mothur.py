...
def make_batch(stability_files_name,batch_file,mothur_ref_dir,control_list):
 ...
 write_list.extend([str('make.contigs(file='+stability_files_name+', processors=4)\n')])
 ...
 write_list.extend([str('align.seqs(fasta=current, reference='+mothur_ref_dir+'silva.v4.fasta)\n')])
 ...
 write_list.extend([str('remove.seqs(fasta=current, accnos=current)\n')])
...
# remove control groups
 ...
 write_list.extend([str('classify.seqs(fasta=current, count=current, reference='+mothur_ref_dir+'trainset9_032012.pds.fasta, taxonomy=trainset9_032012.pds.tax, cutoff=80)\n')])
 write_list.extend([str('cluster.split(fasta=current, count=current, taxonomy=current, splitmethod=classify, taxlevel=4, cutoff=0.15)\n')])
...
def cmd_line(mothur_path,batch_file_path,mothur_output_path):
 mothur_command = str(mothur_path+' '+batch_file_path+' > '+mothur_output_path)
