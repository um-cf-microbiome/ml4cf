...
# write_list.extend([str('classify.seqs(fasta=current, count=current, reference='+mothur_ref_dir+'trainset9_032012.pds.fasta, taxonomy='+mothur_ref_dir+'trainset9_032012.pds.tax, cutoff=80)\n')])
 ...
 write_list.extend([str('make.shared(list='+mothur_output_path+'stability.trim.contigs.good.unique.good.filter.unique.precluster.pick.pick.pick.opti_mcc.unique_list.list, count=current, label=0.03)\n')])
